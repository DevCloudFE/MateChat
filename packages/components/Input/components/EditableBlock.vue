...
import { inject, nextTick, onMounted, type PropType, ref, watch } from 'vue';
import {
  DisplayType,
...
const getCurrentInputValue = () => {
  ...
  if (themeTag.value.themeTagText) {
    const prefixSpan = editableDivRef.value?.querySelector(
...
const handleDivKeydown = (e: KeyboardEvent) => {
  const shiftKey =
    props.submitShortKey === SubmitShortKey.Enter
      ? !e.shiftKey
      : props.submitShortKey === SubmitShortKey.ShiftEnter
        ? e.shiftKey
        : false;
  ...
  if (['ArrowLeft', 'ArrowRight', 'Backspace', 'Delete'].includes(e.key)) {
    ...
    const direction =
      e.key === 'ArrowLeft' || e.key === 'Backspace' ? 'previous' : 'next';
    ...
    if (targetNode && targetNode.nodeType === Node.ELEMENT_NODE) {
      ...
      if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
        ...
        newRange.setStart(
          editSpanDom,
          direction === 'previous' ? editSpanDom.childNodes.length : 0,
        );
...
@import "@matechat/common/Input/common/editable-block.scss";
