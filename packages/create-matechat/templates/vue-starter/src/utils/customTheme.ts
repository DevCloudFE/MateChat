...
import { ColorHierarchyMap } from "./color-hierarchy.ts";
...
export class CustomThemeService {
  ...
  constructor(
    colorHierarchy: any = ColorHierarchyMap,
    themeDataLight: any = infinityTheme.data,
    themeDataDark: any = galaxyTheme.data
  ) {
  ...
  private updateColor(
    ...
    while (changeStack.length) {
      ...
      if (count < changeKeys.length) {
        ...
        const { mode, offset } = this.getColorEffectOffset(
          extendsColor,
          colorOffset,
          effect
        );
      ...
      colorKeys.forEach((colorName) => {
        if (handleKey === pattern[colorName].extends) {
          if (
            !(changeStack.indexOf(colorName) > -1) &&
            !(changeKeys.indexOf(colorName) > -1)
          ) {
  ...
  // 填补未指定颜色的项，保证主题色板完整。
  private fillEmptyColor(
    pattern: IColorHierarchy,
  ...
  private getColorEffectOffset(
    source: Color,
    colorOffset: IColorOffset | any,
    effect?: IEffect
  ) {
    let result: any = {};
    switch (effect) {
  ...
  private getColorValue(source: Color, offset: any, mode: "hsl" | "hsv") {
    ...
    const value = {
      ...
      [key[1]]:
        offset[key[1] + "p"] > 0
          ? sourceModel.color[1] +
            (offset[key[1] + "p"] * (100 - sourceModel.color[1])) / 100
          : sourceModel.color[1] +
            (sourceModel.color[1] * offset[key[1] + "p"]) / 100,
      [key[2]]:
        offset[key[2] + "p"] > 0
          ? sourceModel.color[2] +
            (offset[key[2] + "p"] * (100 - sourceModel.color[2])) / 100
          : sourceModel.color[2] +
            (sourceModel.color[2] * offset[key[2] + "p"]) / 100,
      a:
        offset.ap! > 0
