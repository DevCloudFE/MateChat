...
以下是一个用于生成 Vue 适配组件的提示词模板。通过提供 Svelte 组件的相关信息，AI 将自动生成 Vue 适配组件代码。
...
请为该 Svelte 组件生成一个 Vue 适配组件，要求：
1. Vue 组件的 props 与 Svelte 组件的 props 保持一致。svelte组件的props就是组件中定义的export let 相关变量。
2. Vue 组件的事件通过 `emit` 抛出，事件需要通过props 传递到 Svelte 组件里面执行，事件执行回调触发vue组件的emit。
...
请为该 Svelte 组件生成一个 Vue 适配组件，要求：
1. Vue 组件的 props 与 Svelte 组件的 props 保持一致。
2. Vue 组件的事件通过 `emit` 转发到 Svelte 组件。
3. Vue 组件的插槽通过 Vue 的 `useSlots` 传递给 Svelte 组件。
4. Vue 组件在 `onMounted` 时初始化 Svelte 组件，并在 `onBeforeUnmount` 时销毁 Svelte 组件。
5. Vue 组件的样式文件保持空白，使用 `scoped`。

生成的 Vue 组件文件名为 `Button.vue`，请输出完整代码。
```
